...
impl Parser {
    ...
    fn parse_port_decls_and_body_with_ansi_ports(&mut self, params: &mut HashMap<String, u64>, ansi_port_names: Vec<String>, ansi_port_dirs: Vec<Option<PortDir>>) -> (Vec<(String, PortDir, Option<Range>)>, Vec<ModuleItem>) {
        ...
        // read additional port direction decls (non-ANSI style)
        loop {
            match self.toks.get(self.pos) {
                Some(Token::Input) => { self.pos += 1; let w = self.parse_range_opt(); for n in self.parse_id_list() { if !declared_names.contains(&n) { port_decls.push((n, PortDir::Input, w.clone())); } } }
                Some(Token::Output) => {
                    self.pos += 1;
                    let is_reg = self.at(Token::Reg);
                    if is_reg { self.pos += 1; }
                ...
                Some(Token::Inout) => { self.pos += 1; let w = self.parse_range_opt(); for n in self.parse_id_list() { if !declared_names.contains(&n) { port_decls.push((n, PortDir::Inout, w.clone())); } } }
                _ => break,
            }
        }
